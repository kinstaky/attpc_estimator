...
class LabelRepository:
    ...
    def list_labeled_trace_keys(self, run: int) -> dict[tuple[int, int], tuple[str, str]]:
    ...
    def get_label(self, run: int, event_id: int, trace_id: int) -> StoredLabel | None:
        row = self.connection.execute(
            """
            SELECT family, label
            FROM trace_labels
    ...
    def has_label(self, run: int, event_id: int, trace_id: int) -> bool:
        row = self.connection.execute(
            """
            SELECT 1
            FROM trace_labels
