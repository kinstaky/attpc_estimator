...
from attpc_estimator.service.traces import TraceSource
...
def test_review_navigation_prefetches_next_five_traces_asynchronously(tmp_path) -> None:
    ...
    labels = {
        TraceRef(run=10, event_id=1, trace_id=trace_id): StoredLabel(
            family="normal",
            label="0",
        )
    ...
    source = TraceSource.for_review_mode(
        trace_path,
        family="normal",
        label=None,
        labels=labels,
    ...
    try:
        ...
        original_prefetch = source._prefetcher._prefetch_window

        def blocked_prefetch(generation, window_keys):
            started.set()
            allow_prefetch.wait(timeout=1.0)
            return original_prefetch(generation, window_keys)

        source._prefetcher._prefetch_window = blocked_prefetch

        first = source.next_trace()
        assert (first.event_id, first.trace_id) == (1, 0)
        assert started.wait(timeout=1.0)
        assert list(source.trace_cache) == [TraceRef(run=10, event_id=1, trace_id=0)]

        allow_prefetch.set()
        assert source._wait_for_prefetch(timeout=1.0)
        assert set(source.trace_cache) == {
            TraceRef(run=10, event_id=1, trace_id=trace_id) for trace_id in range(6)
        }
...
def test_trace_cache_stays_bounded_to_current_window(tmp_path) -> None:
    ...
    write_hdf5_input(trace_path)
    labels = {
        TraceRef(run=11, event_id=1, trace_id=trace_id): StoredLabel(
            family="normal",
            label="0",
        )
        for trace_id in range(20)
    ...
    try:
        ...
        assert set(source.trace_cache) == {
            TraceRef(run=11, event_id=1, trace_id=trace_id) for trace_id in range(1, 12)
        }
